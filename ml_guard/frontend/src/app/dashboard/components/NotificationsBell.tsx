...
export default function NotificationsBell() {
    ...
    const fetchAlerts = useCallback(async () => {
        try {
            const d = await apiGet<any>("/api/v1/alerts/events?limit=10");
            const list = Array.isArray(d) ? d : [];
            setAlerts(list);

            if (!isOpen) {
                const newCount = list.filter(
                    (a: any) => new Date(a.created_at) > lastChecked
                ).length;
                setUnreadCount(newCount);
            }
        } catch {
            // Silently ignore — alerts are non-critical UI sugar.
            // Backend may be offline or starting; don't spam the console.
        }
    }, [isOpen, lastChecked]);

    useEffect(() => {
        fetchAlerts();
        const interval = setInterval(fetchAlerts, 30000); // 30 s polling
                        ...
                        <div className="max-h-[400px] overflow-y-auto no-scrollbar divide-y divide-white/[0.03]">
                            {alerts.length === 0 ? (
                                <div className="p-10 text-center flex flex-col items-center gap-3">
                                    ...
                                    <p className="text-[9px] font-black uppercase text-slate-700 tracking-widest leading-relaxed">
                                        System is stable.<br />No active alerts.
                                    </p>
                                </div>
                            ) : (
                                alerts.map((alert) => (
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[9px] text-slate-500 font-medium truncate">
                                                    {alert.rule_name || "System Monitor"}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
