#!/usr/bin/env python3
"""
ML Guard Demo with Real Data
Demonstrates ML Guard working with actual customer churn dataset and model
"""
...
async def demo_with_real_data():
    ...
    try:
        ...
        if not data_dir.exists():
            print("❌ Data directory not found. Run create_dataset.py first!")
            return
        ...
        if not (model_dir / "churn_model.pkl").exists():
            print("❌ Model file not found. Run create_dataset.py first!")
            return
        ...
        print(f"Execution Time: {test_run.execution_time_seconds:.2f} seconds")
        print(f"Average Test Time: {(test_run.execution_time_seconds / len(test_run.results)):.3f} seconds per test")
        print()
...
def prepare_dataset(df, feature_cols, encoders):
    """Prepare dataset for ML testing by encoding categorical features."""
    df_prepared = df.copy()
