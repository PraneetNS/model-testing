...
class DataQualityTests:
    ...
    def _test_missing_values(self, test_config: Dict[str, Any], datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message = ".2f" if status == "passed" else ".2f"
    ...
    def _test_class_balance(self, test_config: Dict[str, Any], datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message = ".1f" if status == "passed" else ".1f"
...
class StatisticalStabilityTests:
    ...
    def _calculate_psi(self, train_values: pd.Series, val_values: pd.Series, bins: int = 10) -> float:
        ...
        try:
            train_bins = pd.qcut(train_values, q=bins, duplicates='drop')
            bin_edges = train_bins.cat.categories

            # Calculate percentages for training data
            train_percentages = []
            for category in bin_edges:
                mask = train_values.between(category.left, category.right)
                percentage = mask.sum() / len(train_values)
                train_percentages.append(percentage)

            # Calculate percentages for validation data
            val_percentages = []
            for category in bin_edges:
                mask = val_values.between(category.left, category.right)
                percentage = mask.sum() / len(val_values)
                val_percentages.append(percentage)

            # Calculate PSI
            psi = 0
            for train_pct, val_pct in zip(train_percentages, val_percentages):
                if train_pct > 0 and val_pct > 0:
                    psi += (val_pct - train_pct) * np.log(val_pct / train_pct)

            return psi

        except Exception:
            # Fallback to simple binning if quantile binning fails
            all_values = pd.concat([train_values, val_values])
            ...
            train_pct = train_hist / len(train_values)
            val_pct = val_hist / len(val_values)

            psi = 0
            for t_pct, v_pct in zip(train_pct, val_pct):
                if t_pct > 0 and v_pct > 0:
                    psi += (v_pct - t_pct) * np.log(v_pct / t_pct)
...
class ModelPerformanceTests:
    ...
    def run_test(self, test_config: Dict[str, Any], model_artifact: Any, datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if test_type == "accuracy_threshold":
            return self._test_accuracy_threshold(test_config, model_artifact, datasets)
        elif test_type == "precision_threshold":
            return self._test_precision_threshold(test_config, model_artifact, datasets)
        elif test_type == "recall_threshold":
            return self._test_recall_threshold(test_config, model_artifact, datasets)
        elif test_type == "f1_threshold":
            return self._test_f1_threshold(test_config, model_artifact, datasets)
        elif test_type == "roc_auc_threshold":
            return self._test_roc_auc_threshold(test_config, model_artifact, datasets)
        else:
            return {
                "status": "failed",
...
class RobustnessTests:
    ...
    def _test_prediction_stability(self, test_config: Dict[str, Any], model: Any, datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message = ".1%" if status == "passed" else ".1%"
...
class BiasFairnessTests:
    ...
    def _test_disparate_impact(self, test_config: Dict[str, Any], model: Any, datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get predictions
        feature_cols = [col for col in df.columns if col not in [target_column, protected_attribute]]
        X = df[feature_cols]
