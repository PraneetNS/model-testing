...
async def _get_active_model_ids() -> list:
    ...
    async with SessionLocal() as db:
        try:
            stmt = select(PredictionLog.model_id).distinct()
...
def run_hourly_drift_scan(self):
    """
...
async def _run_drift_scan():
    model_ids = await _get_active_model_ids()
    ...
    for model_id in model_ids:
        async with SessionLocal() as db:
            try:
                ...
                result = await analyzer.analyze(window_hours=1)
                if result:
                    results.append({
                        "model_id": model_id,
    ...
    for model_id in model_ids:
        async with SessionLocal() as db:
            try:
                ...
                prev_snap = res.scalars().first()
                baseline = prev_snap.metrics if prev_snap else None

                tracker = PerformanceTracker(db, model_id)
                result = await tracker.compute_snapshot(window_hours=6, baseline_metrics=baseline)
                if result:
